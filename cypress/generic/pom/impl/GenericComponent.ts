...
} from "../../../shared/assertions/chainable"; // Import assertion utilities
...
class GenericComponent implements IGenericComponent {
    ...
    */
    ...
    /**
     * The component itself can also contain other sub components.
     * 
     * The sub component key is useful as a reference for intercepting requests to the base data API.
     * 
     * The sub component map key is used as a reference for a specific element.
     * 
    ...
    */
    ...
    constructor(
        ...
        if (as === COMPONENT_TYPE.PAGE) {
            this.cyElement = () => cy.get('body');
        } else {
    ...
    setElements(
        ...
        Object.entries(elementsSelectors).forEach(([elementKey, selectorFn]) => {
            const dataBaseAPI = dataBaseUrls
                ? dataBaseUrls[elementKey]?.()
                : undefined;
    ...
    async setButtons(
        ...
        Object.entries(buttonSelectors).forEach(([buttonKey, selectorFn]) => {
            const dataBaseAPI = dataBaseUrls
                ? dataBaseUrls[buttonKey]?.()
                : undefined;

            const menuSelectorFn = menuSelectors
                ? menuSelectors[buttonKey]
                : undefined;
            ...
            genericButton.entry = selectorFn().type as BUTTON_ENTRY
    ...
    getComponentMenu(): GenericMenu | undefined {
        const element = this.elements.get(this.getKey() + '_menu');
        if (element && element.as === COMPONENT_TYPE.MENU) {
            return element as GenericMenu;
        } else {
            console.error(`Menu '${this.getKey() + '_menu'}' not found or not of type 'MENU'.`);
            return undefined;
        }
    }
    ...
    async setMenus(
        ...
        Object.entries(menuSelectors).forEach(([menuKey, selectorFn]) => {
            const item = items
                ? items[menuKey]
                : undefined;

            const dataBaseAPI = dataBaseUrls
                ? dataBaseUrls[menuKey]?.()
                : undefined;
        ...
        if (values) {
            ...
            values.forEach((text: string) => {
                assertElementContains(
                    this.cyElement(),
                    text,
                    `Failed to find ${text} in ${this.getKey()}.`
                ).click(
                    { force: true }
                );
            })
