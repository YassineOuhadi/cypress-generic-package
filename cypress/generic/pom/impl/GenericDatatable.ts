...
class GenericDatatable extends GenericComponent implements IGenericDatatable {
     ...
     * Selector for the page size select field : The key '_pagesize' key indicates the page size select field.
     * Selector for the total label : he key '_total' key indicates the total label.
    */
    ...
    addColumns(
        ...
        Object.entries(columnSelectors).forEach(([columnKey, selectorFn], index) => {
            const filterComponent = filters ? filters[columnKey] : undefined;
            let sortbuttonFn;

            // Check if sortbuttons are provided and if a button with the same key exists
            if (sortbuttons && sortbuttons[columnKey]) {
                sortbuttonFn = sortbuttons[columnKey];
            }
    ...
    assertDatatableHasRows(
        ...
        this.cyElement().wait(
            ...
            const numberOfRows = Cypress.$(rows).length;
