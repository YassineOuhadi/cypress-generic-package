// cypress/shared/chainable/chainableAssertions.ts
...
const assertURLIncludes = (
  ...
  return cy.url().then((url) => {
    const passed = url.includes(partialURL);
    if (!passed) {
      throw new Error(
        errormessage ?? `Expected URL to include "${partialURL}", but received "${url}"`
      );
    }

    // Cypress.log pour message visuel
    Cypress.log({
      name: "assertURLIncludes",
      message: `URL includes "${partialURL}"`
    });

    // retourne un boolean typé correctement
    return passed as boolean;
  });
};
